 ...
 **************************************************************************************
   ...
   Filename:
    $Workfile: PNS_simpleConfig.c $
   Last Modification:
    $Author : AB$
    $Modtime: 16.06.17 15:45 $
    $Revision$
   ...
   Description:
     Linux I/O test application configuring netX as PROFINET IO device with 
     10 bytes input and 4 bytes output across the 'cifX API' (netX driver API)

   Preconditions:
     A PROFINET controller addresses IO devices over unique device names. After
     the demo has been started the device waits for the assignment of its device name
     (default 'netx51repns') over the DCP protocol across Ethernet.
...
int32_t ChannelDemo()
{
  ...
  if(CIFX_NO_ERROR == lRet)
  {
    ...
    } else
    {
      ...
      printf("\nSending PROFINET IO device configuration request!\n");
      printf("\nConnect your PROFINET IO controller now to any of the two Industrial Ethernet ports.\n");
      printf("\nThe demo is waiting for the controller starting communication.\n");
      printf("\nDuring this time the device is able to receive a device name via DCP protocol over Ethernet.\n");
	...
	if(tAppData.fRunning == true)
        {
          ...
          if(CIFX_NO_ERROR != (lRet = xChannelIORead(hChannel, 0, 0, sizeof(APP_INPUT_DATA_T), tAppData.tInputData.abApp_Inputdata, 10)))
          {
          ...
          } else
          {
            ...
            memcpy(tAppData.tOutputData.abApp_Outputdata,tAppData.tInputData.abApp_Inputdata, sizeof(APP_OUTPUT_DATA_T));

            if(CIFX_NO_ERROR != (lRet = xChannelIOWrite(hChannel, 0, 0, sizeof(APP_OUTPUT_DATA_T), tAppData.tOutputData.abApp_Outputdata, 10)))
            {
	      if(CIFX_DEV_NO_COM_FLAG != lRet)
              {
...
int main(int argc, char* argv[])
{
  ...
  printf("Acceptance for a device name over DCP protocol is enabled in the device after the demo has been started.\n");
  printf("If not configured otherwise the default device name is 'netx51repns' (as preset in the provided GSDML file).\n"); 
  ...
  while( (lRet = stat("/opt/cifx/plugins/netx-spm/config0",&buffer)) != 0) {
    ...
    if( system("dpkg -i ./driver/netx-drv-1.1.0.deb") == -1 ) {
      printf("Installing the driver failed. Check if driver package 'netx-drv-1.1.0.deb' is located in folder './driver'(relative path to demo app).\n");
      return 0;
    }
  }

  /* check if the PROFINET IO device firmware has been installed properly on the system, else install its package */
  while( (lRet = stat("/opt/cifx/deviceconfig/FW/channel0/R160D000.nxf",&buffer)) != 0) {
    ...
    if( system("dpkg -i ./firmwares/netpi-pns-3.12.0.2.deb") == -1) {
      printf("Installing the firmware failed. Check if firmware 'netpi-pns-3.12.0.2.deb' package is located in folder './firmwares'(relative path to demo app).\n");
      return 0;
    }
  }
