 ...
 **************************************************************************************
   ...
   Filename:
    $Workfile: ECS_simpleConfig.c $
   Last Modification:
    $Author : AB$
    $Modtime: 21.07.17 15:45 $
    $Revision$
   ...
   Description:
     Linux I/O test application configuring netX as EtherCAT slave with 
     10 bytes input and 4 bytes output data across the 'cifX API' (netX driver API)
...
int32_t ChannelDemo()
{
  ...
  if(CIFX_NO_ERROR == lRet)
  {
    ...
    } else
    {
      ...
      printf("\nSending EtherCAT slave configuration request!\n");
      printf("\nConnect your EtherCAT master now to any of the two Industrial Ethernet ports.\n");
